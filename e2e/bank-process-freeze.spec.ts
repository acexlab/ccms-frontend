...
test.describe('Bank Case Processing Flow', () => {
  test('should log in as Bank Officer, run batch, open awaiting cases, submit freeze response, and verify completed status', async ({ page }) => {
    // 1. Login
    ...
    // 5. Open "Awaiting Action" tab case
    // In our test seeding, Case E (CCMS-TEST-0005) is Awaiting Action (status: AccountValidated).
    ...
    // 8. Verify redirected back to detail and status is now FreezeApplied
    await expect(page).toHaveURL(/\/bank\/cases\/CCMS-TEST-0005/);
